...
from dataset import load_data_hybrid
from config import BASE_PATH, model_dirs
from model_templates.HybridModel.models import SetTransformer
...
def run_epoch_sequential(model, optimizer, loader, loss_meter, acc_meter, criterion, is_training, concept_model=None):
    ...
    for idx, data in enumerate(loader):
        ...
        if idx > 2:
            print(f'shorten by 2 on idx: {idx}')
            inputs = inputs[:inputs.shape[0]-2]
            class_labels = class_labels[:class_labels.shape[0]-2]

        print(f'bs: {class_labels.shape}')
...
def train(model, args):
    ...
    if os.path.exists(args.log_dir):  # replace old fold by a new one
    ...
    val_loader = load_data_hybrid([val_data_path], args.use_attr, args.no_img, args.batch_size,
    ...
    for epoch in range(0, args.epochs):
        ...
        with torch.no_grad():
            if args.exp == 'Independent_CtoY':
                print('error val independent')
                exit()

            elif args.exp == 'Sequential_CtoY':
                val_loss_meter, val_acc_meter = run_epoch_sequential(model, optimizer, val_loader,
                                                                     val_loss_meter, val_acc_meter, criterion,
                                                                     is_training=False, concept_model=concept_model)
            else:
                print('error val else')
                exit()
...
def parse_arguments():
    ...
    parser.add_argument('-exp', type=str,
                        choices=['Concept_XtoC', 'Concept_XtoC_Resnet', 'Independent_CtoY', 'Sequential_CtoY',
                                 'Standard', 'Joint'],
