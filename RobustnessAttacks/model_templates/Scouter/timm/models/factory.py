...
from .layers.config import set_layer_config
...
def create_model(
    ...
    Args:
        ...
        scriptable (bool): set layer config so that model is jit scriptable (not working for all model_templates yet)
    ...
    # Only EfficientNet and MobileNetV3 model_templates have support for batchnorm params or drop_connect_rate passed as args
    ...
    # Parameters that aren't supported by all model_templates should default to None in command line args,
    # remove them if they are present and not set so that non-supporting model_templates don't break.
