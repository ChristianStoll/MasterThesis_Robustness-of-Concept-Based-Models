...
class SparseFool(Attack):
    ...
    """
    def __init__(self, model, steps=10, steps_deepfool=2, lam=3, overshoot=0.02, use_segmentation=False):
    ...
    def forward(self, images, labels, segmentations=None):
        ...
        while (True in correct) and (curr_steps < self.steps):
            for idx in range(batch_size):
                ...
                if self.use_segmentation and segmentations is not None:
                    segmentations = segmentations.clone().detach().to(self.device)
                    adv_image = image + segmentations[idx] * self.lam * (adv_image - image)
                else:
                ...
                if self.use_segmentation:
                    adv_image = self._linear_solver(image, segmentations[idx] * grad, adv_image)
                    adv_image = image + segmentations[idx] * (1 + self.overshoot) * (adv_image - image)
                else:
